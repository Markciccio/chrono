...
<style>html,body,#map{margin:0;width:100%;height:100%;background:#071720}.leaflet-tile-pane{filter:saturate(.62) brightness(.78) contrast(1.12)}.leaflet-control-attribution{font-size:9px;background:rgba(4,14,20,.78)!important;color:#b8d7e2}.leaflet-control-zoom a{background:#081c26!important;color:#48cdff!important;border-color:#1c94be!important}.speed-label{background:rgba(3,14,20,.92)!important;border:1px solid #d8f3ff!important;border-radius:3px!important;color:#fff!important;font-weight:bold!important;font-size:10px!important;padding:2px 4px!important;box-shadow:none!important}.speed-label:before{display:none!important}.timing-label{border-radius:3px!important;color:#071720!important;font-weight:bold!important;font-size:10px!important;padding:2px 4px!important;box-shadow:none!important}.timing-label:before{display:none!important}.timing-finish{background:#ffe000!important;border:1px solid #fff4a0!important}.timing-s1{background:#ff9f1c!important;border:1px solid #ffd295!important}.timing-s2{background:#b983ff!important;border:1px solid #dec5ff!important}.timing-s3{background:#f562c8!important;border:1px solid #ffc0eb!important}</style></head>
...
window.setLine=function(points){if(line)map.removeLayer(line);line=null;if(points)line=L.polyline(points,{color:'#ffe000',weight:6}).bindTooltip('TRAGUARDO',{permanent:true,direction:'center',className:'timing-label timing-finish'}).addTo(map);};
window.setSectors=function(sectors){const colors=['#ff9f1c','#b983ff','#f562c8'];sectorLines.forEach(s=>map.removeLayer(s));sectorLines=sectors.map((points,index)=>L.polyline(points,{color:colors[index]||'#b983ff',weight:5,dashArray:'8 7'}).bindTooltip('S'+(index+1),{permanent:true,direction:'center',className:'timing-label timing-s'+(index+1)}).addTo(map));};
