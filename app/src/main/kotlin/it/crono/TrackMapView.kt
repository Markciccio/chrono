...
<style>html,body,#map{margin:0;width:100%;height:100%;background:#071720}.leaflet-tile-pane{filter:saturate(.62) brightness(.78) contrast(1.12)}.leaflet-control-attribution{font-size:9px;background:rgba(4,14,20,.78)!important;color:#b8d7e2}.leaflet-control-zoom a{background:#081c26!important;color:#48cdff!important;border-color:#1c94be!important}</style></head>
<body><div id='map'></div><script>
const map=L.map('map',{zoomControl:true}).setView([45.656,8.493],14);
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap contributors'}).addTo(map);
let track,line,fix,sectorLines=[],hasFit=false;
