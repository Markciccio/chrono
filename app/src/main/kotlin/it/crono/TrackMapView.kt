...
window.setSpeedMarkers=function(markers){speedMarkers.forEach(m=>map.removeLayer(m));speedMarkers=markers.map(item=>{const braking=item.kind==='braking';const color=braking?'#ff9f1c':'#25d7ad';const label=(braking?'MIN ':'MAX ')+item.speed+' km/h';return L.circleMarker([item.lat,item.lon],{radius:7,color:'#fff',weight:2,fillColor:color,fillOpacity:1}).bindTooltip(label,{permanent:true,direction:'top',offset:[0,-8],className:'speed-label'}).addTo(map);});};
